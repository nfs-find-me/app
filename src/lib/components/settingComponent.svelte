...
{#if isLogged}
    ...
    <section class="flex flex-col laptop:flex-row">
        <nav class="w-full flex flex-col bg-white-secondary border-blue-dark border-b-4 laptop:border-r-4 laptop:w-1/5 justify-between items-center">
            <ul class="py-10 px-4 flex flex-row laptop:flex-col gap-4">
                <li on:click={() => updateOngletInfo(1, "Mon compte")} class="p-4 flex flex-col laptop:flex-row gap-4 items-center cursor-pointer">
                    <i class="fa-regular fa-user text-xl text-text-contrast font-bold"></i>
                    <span class="text-lg text-text-contrast">Mon compte</span>
                </li>
                <li on:click={() => updateOngletInfo(2, "Sécurité")} class="p-4 flex flex-col laptop:flex-row gap-4 items-center cursor-pointer">
