<script lang="ts">
    export let isLogged:Boolean;
</script>

<!-- Desktop Nav -->
  
  <nav class="tablet-max:hidden flex justify-between items-center p-4">
    <div class="flex flex-row gap-2 items-center hover:cursor-pointer">
        <img class="w-20" src="favicon.svg" alt="logo">
        <h2 class="text-xxl font-bold text-blue">FindMe</h2>
    </div>

    <ul class="flex gap-6 font-bold text-lg">
        <li class="hover:cursor-pointer hover:text-blue">Accueil</li>
        {#if isLogged }
            <li class="hover:cursor-pointer hover:text-blue">Explorer</li>
            <li class="hover:cursor-pointer hover:text-blue">Classement</li>
        {:else}
            <li class="hover:cursor-pointer hover:text-blue">A propos</li>
        {/if}
        
    </ul>
    
    <div class="flex gap-6 items-center">
        {#if isLogged }
            <i class="fa-solid fa-gear text-40 hover:cursor-pointer hover:text-blue"></i>
            <img class="w-16 h-16 rounded-50 hover:cursor-pointer" src="profil_picture.jpg" alt="image de profil">
        {:else}
            <button class="btn-primary">Se connecter</button>
            <button class="btn-secondary">S'inscrire</button>
        {/if}
    </div>
</nav>

<!-- Phone Nav -->

<nav class="tablet:hidden fixed z-50 w-full h-20 max-w-lg -translate-x-1/2 shadow-xl bg-white-secondary rounded-full bottom-4 left-1/2">
    <div class="grid h-full max-w-lg grid-cols-5 mx-auto ">
        <button class="inline-flex flex-col items-center justify-center px-5 rounded-l-full">
            <i class="fa-solid fa-house text-xxl text-text-contrast"></i>
        </button>

        <button class="inline-flex flex-col items-center justify-center px-5 rounded-l-full">
            <i class="fa-solid fa-map text-xxl text-text-contrast"></i>
        </button>

        <button class="inline-flex flex-col items-center justify-center px-5 rounded-l-full">
            <i class="fa-solid fa-circle-plus text-60 text-blue"></i>
        </button>

        <button class="inline-flex flex-col items-center justify-center px-5 rounded-l-full">
            <i class="fa-solid fa-trophy text-xxl text-text-contrast"></i>
        </button>

        <button class="inline-flex flex-col items-center justify-center px-5 rounded-l-full">
            <i class="fa-solid fa-gear text-xxl text-text-contrast"></i>
        </button>

    </div>
</nav>

